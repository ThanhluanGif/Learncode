...
| DBG-009 | ERR-014 Sites auth/dispatch | v4/v2/v1 + v5 đều platform `500` | Chờ auth/dispatch project phục hồi và live QA lại | BLOCKED |
...
- `DBG-009` (2026-07-13): ba version độc lập đều deployment `succeeded` nhưng production hostname trả cùng trang platform `500`; OpenAI Status có incident Sites cùng thời điểm. Sau khi status chuyển `Resolved`, version 4 được redeploy và version 5 được lưu/đóng gói/deploy mới nhưng vẫn 500. Trình duyệt xác nhận `/signin-with-chatgpt` cũng 500. Vòng này giữ `BLOCKED` cho đến khi world-state live xanh.
